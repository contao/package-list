...
use Contao\PackageList\Indexer\Package\Factory;
use Contao\PackageList\Indexer\Package\Package;
 ...
 */
#[AsCommand('package-index', 'Indexes package metadata')]
class IndexCommand extends Command
{
    ...
    private function deleteRemovedPackages(bool $dryRun): void
    {
        ...
        foreach ($this->indexData as $item) {
            if (isset($item['name']) && !isset($this->packages[$item['name']])) {
        ...
        $this->loupe->deleteDocuments($packagesToDeleteFromIndex);
    }
    ...
    private function indexPackages(bool $dryRun, bool $updateStats): void
    {
        ...
        foreach (array_chunk($this->packages, 100) as $chunk) {
            ...
            if (!$dryRun) {
                $this->loupe->addDocuments($objects);
            }
        }
    ...
    private function initIndex(bool $clearIndex): void
    {
        ...
        $params = BrowseParameters::create()->withLimit(1000);
        $page = 1;
        $data = [];
        ...
        do {
            ...
            $results = $this->loupe->browse($params);
            $data[] = $results->toArray()['hits'];
        } while ($page < $results->getTotalPages());

        $this->indexData = array_merge(...$data);
    }
