...
namespace Contao\PackageList\Indexer\Package;

use Composer\Semver\Constraint\Constraint;
use Composer\Semver\Constraint\MultiConstraint;
use Composer\Semver\Intervals;
use Composer\Semver\VersionParser;
use Contao\PackageList\Command\IndexCommand;
use Contao\PackageList\Indexer\MetaDataRepository;
use Contao\PackageList\Indexer\Packagist;
...
class Factory
{
    ...
    private function setDataForPrivate(Package $package): void
    {
        ...
        $package->setAbandoned((bool) ($data['abandoned'] ?? false), \is_string($data['abandoned'] ?? null) ? $data['abandoned'] : null);
    ...
    private function setBasicDataFromPackagist(array $data, Package $package): void
    {
        ...
        $package->setAbandoned((bool) ($data['packages']['abandoned'] ?? false), \is_string($data['packages']['abandoned'] ?? null) ? $data['packages']['abandoned'] : null);
    ...
    private function findLatestVersion(array $versions): array
    {
        $latest = array_reduce(
            array_keys($versions),
            static function (string|null $prev, string $curr) use ($versions) {
                if (null === $prev) {
                    return $curr;
                }
    ...
    private function buildContaoConstraint(array $versions): string|null
    {
        ...
        foreach ($versions as $package) {
            if ('stable' !== VersionParser::parseStability($package['version'])) {
                continue;
            }
