<template>
    <composer-package :data="data" :hide-packagist="isPrivate">
        ...
        <a class="widget-button widget-button--primary widget-button--link" target="_blank" :href="data.homepage" slot="actions" v-if="data.homepage">{{ 'ui.package.homepage' | translate }}</a>
        <a class="widget-button widget-button--link" target="_blank" :href="`https://packagist.org/packages/${data.name}`" slot="actions" v-if="!isPrivate">{{ 'ui.package.packagist' | translate }}</a>
    </composer-package>
</template>
